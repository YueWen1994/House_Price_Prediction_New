...
1) For tax/area related missing data, because there are relationship between different tax featres, it works the same way for the area  feature. Therefore, we used  the related feature to impute the missing data by simple calculation.

2) For zip code/city, what we  did here is to use longitude/latitude to find its nearest neighbor and them use the same zip/city.
...
Because it takes long to run  the imputation part,  I save the result to the csv, and we don't evaluate the code below anymore. Instead, we directly import the data generated.
```{r,include=FALSE}
...
#anyinformation, and we have  to bother to impute it
...
Note: the reasons I came up with the coming three features is due to visualization result by plot mean(error)~ category.
...
Here, we do not list every linear model built. Instead, we just give an example to show the basic idea of how to choose the model using residual plot  diagnosis.
```{r,warning=FALSE,message=FALSE}
